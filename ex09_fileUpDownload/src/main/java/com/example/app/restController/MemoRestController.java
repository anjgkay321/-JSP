...
public class MemoRestController {
	...
	public ResponseEntity<MemoDto> get(@PathVariable int id) {
		...
		return new ResponseEntity<MemoDto>(dto, HttpStatus.OK);
	}
...
//		}
	// 메모쓰기
	@PostMapping("/post") // http://localhost:8090/rest/memo/post
	public void add(@RequestBody MemoDto dto) throws Exception {
		...
		//메모쓰기
		@GetMapping("/add")	//http://localhost:8080/app/memo/add
		public void add_g(MemoDto dto) throws Exception {
			log.info("GET /memo/add_rest_get.."+dto);
			memoService.registrationMemo(dto);
		}
	@PostMapping("/add") // http://localhost:8090/rest/memo/post
	public void add_get( MemoDto dto) throws Exception {
		log.info("POST /memo/add_rest_post.." + dto);
		memoService.registrationMemo(dto);
	}
